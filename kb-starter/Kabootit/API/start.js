 ...
 * @return {JSON}
 */
function startController (request, response) {
	var 
		call,
		offset,
		version,
	...
	try {
		...
		version = Number(version) == version ? version : START.activeVersion;
		...
		fx = eval('START.version[' + (version - 1) + '].' + method);
	}
	catch (e) {
	...
	if (request.host.indexOf('localhost') == 0 || 
		// whitelisted host
		START.accessHost.indexOf(request.remoteAddress) != -1 ||
		// debug token present
		START.accessToken.indexOf(params.token) != -1) {
...
var actionsAllowed = XHR.post(URL, "security", "START").result;
...
///// TO CREATE NEW API
//
// 1) Find/replace in this file 'START' with whatever the API you are creating
 ...
 */
START.accessToken.push('1111');

/**
 * Whitelisted hosts
 */
// START.accessHost.push('8.8.4.4');

/**
 * Set active version of API
 */
//START.activeVersion = 2;
 
/**
 * Pull in numbered versions (START_v1.js, START_v2.js, ..., START_vn.js)
 */
init('START',START);