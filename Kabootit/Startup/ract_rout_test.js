function ractive3(request, response) {
	...
	var wakPage = Folder(ds.getModelFolder().path + 'WebFolder/' + 'ractive/A3_routing' + '.waPage');
				...
				try {
					...
					if (wakPage.exists) {
						...
						if (index != -1) {
							...
							index += 28;
							html = html.substring(0,index) + ' content="/' + 'ractive/A3_routing' + '.waPage/index.package.json"' + html.substring(index, html.length);
						}
					}
...
function ractive(request, response) {
	...
	// it is possible to pass in a fully qualified wakanda page as the homePage name and therefore not trigger wakPage
	var wakPage = Folder(ds.getModelFolder().path + 'WebFolder/' + 'ractive/A2_routing' + '.waPage');
				...
				try {
					...
					if (wakPage.exists) {
						...
						if (index != -1) {
							// 28 is length of meta name blah blah
							index += 28;
							html = html.substring(0,index) + ' content="/' + 'ractive/A2_routing' + '.waPage/index.package.json"' + html.substring(index, html.length);
						}
					}
