...
WAF.proxy.HttpRequest = function (config) {
    ...
    this.send = function (params) {
		...
		params.ontimeout = params.ontimeout || function () {};
        if (!config.ispost) {
            method = 'GET';
        };
		// KABOOTIT: add timeout property to xhr
		if ( config.isasync ) {
        ...
        if (!config.isasync) {
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4) {
					// KABOOTIT: lousy way to detect timeout and send it through to callback
					if ( xhr.responseText === "" ) {
						params.ontimeout({ code: 408, message: "Request Timeout (Wakanda RPC)"})
						return;
					}
                    oResponse = xhr.responseText.replace(/^\s+|\s+$/g, '');
                    if (oResponse !== null) {
...
WAF.proxy.ProxyFactory = {
    ...
    createAsyncFunc : function(name, module) {
        return function () {
			...
			// KABOOTIT: ontimeout
            if (arguments[0].onsuccess || arguments[0].onSuccess || arguments[0].onerror || arguments[0].onError || arguments[0].ontimeout || arguments[0].onTimeout || arguments[0].params ) {
                ...
                request.send({
					...
					ontimeout : arguments[0].ontimeout || arguments[0].onTimeout, // KABOOTIT: ontimeout
					timeout   : arguments[0].timeout // KABOOTIT: timeout
                });
            } else {
                message = new WAF.proxy.JsonMessage ({
                    name        : name,
