 ...
 * @return {JSON}
 */
function srvrmonController (request, response) {
	var 
		call,
		offset,
		version,
	...
	try {
		...
		version = Number(version) == version ? version : SRVRMON.activeVersion;
		...
		fx = eval('SRVRMON.version[' + (version - 1) + '].' + method);
	}
	catch (e) {
	...
	if (request.host.indexOf('localhost') == 0 || 
		// whitelisted host
		SRVRMON.accessHost.indexOf(request.remoteAddress) != -1 ||
		// debug token present
		SRVRMON.accessToken.indexOf(params.token) != -1) {
...
var URL = SECURITY.path('A' + 'C',true);
...
var actionsAllowed = XHR.post(URL, "security", "SRVRMON").result;
...
///// TO CREATE NEW API
//
// 1) Find/replace in this file 'SRVRMON' with whatever the API you are creating
 ...
 */
SRVRMON.accessToken.push('1111');

/**
 * Whitelisted hosts
 */
// SRVRMON.accessHost.push('8.8.4.4');

/**
 * Set active version of API
 */
//SRVRMON.activeVersion = 2;
 
/**
 * Pull in numbered versions (SRVRMON_v1.js, SRVRMON_v2.js, ..., SRVRMON_vn.js)
 */
init('SRVRMON',SRVRMON);