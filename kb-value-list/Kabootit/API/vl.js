 ...
 * @return {JSON}
 */
function vlController (request, response) {
	var
		call,
		offset,
		version,
	...
	try {
		...
		version = Number(version) == version ? version : VL.activeVersion;
		...
		fx = eval('VL.version[' + (version - 1) + '].' + method);
	}
	catch (e) {
	...
	if (request.host.indexOf('localhost') == 0 ||
		// whitelisted host
		VL.accessHost.indexOf(request.remoteAddress) != -1 ||
		// debug token present
		VL.accessToken.indexOf(params.token) != -1) {
...
var actionsAllowed = XHR.post(URL, "security", "VL").result;
...
///// TO CREATE NEW API
//
// 1) Find/replace in this file 'VL' with whatever the API you are creating
 ...
 */
 VL.accessToken.push('1111');

/**
 * Whitelisted hosts
 */
// VL.accessHost.push('8.8.4.4');
...
/**
 * Pull in numbered versions (VL_v1.js, VL_v2.js, ..., VL_vn.js)
 */
init('VL',VL);