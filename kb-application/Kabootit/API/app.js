...
function appController (request, response) {
	...
	try {
		...
		version = Number(version) == version ? version : APP.activeVersion;
		...
		fx = eval('APP.version[' + (version - 1) + '].' + method);
	}
	catch (e) {
	...
	if (request.host.indexOf('localhost') == 0 ||
		// whitelisted host
		APP.accessHost.indexOf(request.remoteAddress) != -1 ||
		// debug token present
		APP.accessToken.indexOf(params.token) != -1) {
...
var actionsAllowed = XHR.post(URL, "security", "APP").result;
...
///// TO CREATE NEW API
//
// 1) Find/replace in this file 'APP' with whatever the API you are creating
 ...
 */
APP.accessToken.push('1111');

/**
 * Whitelisted hosts
 */
// APP.accessHost.push('8.8.4.4');
...
/**
 * Pull in numbered versions (APP_v1.js, APP_v2.js, ..., APP_vn.js)
 */
init('APP',APP);